...
function statusbar {
	...
	echo "$GAP2%{F$CYAN}$GAP1$(mem)$GAP2%{F$RED}$GAP2$(vol){F-}%{r}%$GAP2%{F$GREEN}%{F$GREEN}$GAP1$(cur) %{B-}%{F-}$GAP3$GAP2$GAP2$GAP2%{F$BLUE}  %{B-}%{F-} %{F$BLUE}$(bat)% 
%{F$YELLOW}  %{B-}%{F-} %{F$YELLOW}%$(clock)  "
}
 while true 
 do
	 echo "$(statusbar)"
